...
void ANavesUSFX2026GameMode::BeginPlay()
{
    ...
    ADirEscenarioEspacial* Director = GetWorld()->SpawnActor<ADirEscenarioEspacial>(ADirEscenarioEspacial::StaticClass());

    if (Director)
    {
        AConBuilderFacil* BuilderFacil = GetWorld()->SpawnActor<AConBuilderFacil>(AConBuilderFacil::StaticClass());

        if (BuilderFacil)
        {
            Director->SetBuilder(BuilderFacil);
            Director->ConstruirEscenario();
        }
    }
}
